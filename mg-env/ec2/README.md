...
# With current subnet_id it is designed to read from the statefile and place EC2 instances in the subnet in order. If 1 EC2 instance is created, then it will go in the first subnet listed in the statefile. If 2 EC2 instances are created, then each will go into a different subnet (1st EC2 in the 1st subnet listed, 2nd EC2 in the 2nd subnet listed). If the desired approach is to specify which subnets the EC2 instances go in, it will be required to create a variable in your project for 'subnet_id' and then specify each subnet individually in a list format. ex... ["subnet1","subnet2","subnet3"]
...
| instance\_dns\_names | Instance DNS count | list | n/a | yes |
...
| user\_data | The user data to provide when launching the instance | list | `<list>` | no |
